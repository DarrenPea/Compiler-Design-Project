    ...
    def p_expprime(left: Exp):Parser[PEnv, Exp] = {
        val op_parser = for {
            ...
            op match {
                ...
                case LThan => LThan(left, right)
                case DEq => DEqual(left, right)
            }
        }
        choice(op_parser)(empty(left))
    }

    def p_op:Parser[PEnv, Op] = for {
        tok <- choice(p_plus)(choice(p_minus)(choice(p_mult)(choice(p_lthan)(@@))))
    }
